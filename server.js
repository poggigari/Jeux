/* Trois jeux, un seul service
   ...
   Les trois jeux gardent des sessions totalement separees :
     - Loup-Garou : routes /api/*  et  /sse
     - Imposteur  : routes /imposteur/api/*
     - Memory     : routes /memory/api/*  (module autonome memory-module.js)
*/
...
const mem = require("./memory-module.js");
...
async function lgApi(req, res, url){
  ...
  if(route === "/api/creer"){
    ...
    parties.set(c, {code:c, mjToken: reprise ? b.mjToken : jeton(), mjFlux:[], joueurs:[], pub:{}, prive:{},
      demarree:false, maj:Date.now(), action:null});
  ...
  if(route === "/api/pousser"){                          // le meneur publie l'état
    ...
    if(b.ackAction && p.action && p.action.ts === b.ackAction) p.action = null;
    p.demarree = !!b.demarree;
    diffuserJoueurs(p);            /* jamais vers le meneur : évite la boucle rendu → push → rendu */
    return json(res, {ok:true});
  }

  if(route === "/api/action"){                           // un joueur transmet un choix (ex. la Voyante)
    if(!p) return json(res, {ok:false, erreur:"session introuvable"}, 404);
    const j = p.joueurs.find(function(x){ return x.token === b.token; });
    if(!j) return json(res, {ok:false, erreur:"joueur introuvable"}, 404);
    p.action = {playerId:j.id, type:String(b.type||""), cible: b.cible || null, ts: Date.now()};
    p.maj = Date.now();
    p.mjFlux.forEach(function(r){ ecrire(r, vueMJ(p)); });
    return json(res, {ok:true});
  }
...
function statique(req, res, url){
  ...
  if(f === "/loup-garou" || f === "/imposteur" || f === "/memory"){
...
const serveur = http.createServer(function(req, res){
  ...
  /* Memory */
  if(p.startsWith("/memory/api/")){
    const sub = p.split("/").filter(Boolean).slice(1);   // enleve "memory"
    return mem.api(req, res, url, sub).catch(function(e){ console.error(e); json(res, {ok:false}, 500); });
  }
...
/* menage : une session sans activite depuis 8 h disparait, dans les trois jeux */
setInterval(function(){
  ...
  mem.sessions.forEach(function(s, c){ if(s.maj < seuil) mem.sessions.delete(c); });
}, 30*60*1000);

serveur.listen(PORT, function(){
  console.log("Trois jeux en ligne sur http://localhost:" + PORT);
